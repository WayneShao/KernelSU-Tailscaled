...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build deterministic Magisk Tailscaled ZIP files")
...
def normalized_bytes(path: Path, name: str) -> bytes:
    ...
    if PurePosixPath(name).suffix.lower() in TEXT_SUFFIXES or name.startswith("tailscale/scripts/"):
        data = data.replace(b"\r\n", b"\n")
    return data
...
def build(source: Path, output: Path) -> list[Path]:
    ...
    variants = (
        ("all", ARCHITECTURES, f"Magisk-Tailscaled-v{version}.zip"),
        ("arm", ("arm",), f"Magisk-Tailscaled-arm-v{version}.zip"),
        ("arm64", ("arm64",), f"Magisk-Tailscaled-arm64-v{version}.zip"),
    )
    results = []
